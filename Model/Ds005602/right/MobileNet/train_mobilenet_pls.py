...
class MobileNetV2_1D(nn.Module):
    def __init__(self, in_channels=1, num_classes=1):
        ...
        self.features = nn.Sequential(
            ...
            nn.BatchNorm1d(32),
            nn.ReLU6(inplace=True),
            InvertedResidual1D(32, 32, stride=1, expand_ratio=1),
            InvertedResidual1D(32, 64, stride=2, expand_ratio=6),
            InvertedResidual1D(64, 128, stride=2, expand_ratio=6),
        )
...
def run_pipeline():
    ...
    train_df = pd.read_csv('../Ds005602_Right_train_coef_features.csv')
    test_df = pd.read_csv('../Ds005602_Right_test_coef_features.csv')
    ...
    print(f"Running MobileNetV2_1D {n_splits}-Fold CV with Checkpointing (Batch Size: {batch_size})...")
    ...
    plt.title(f'MobileNetV2_1D Confusion Matrix (Acc: {test_acc:.2%})')
    plt.tight_layout()
